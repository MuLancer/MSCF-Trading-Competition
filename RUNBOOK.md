...
端口:Client **16630** / DMA 16635。Trader ID 是 **`tqdu`**(不是波动率案例的 `tqdu-1`)。
...
核对三件事:**字段名**、**限额权重**、**当前套利边际**。

> 权重和上限是运行时从 API 读的,不是写死的。练习服务器上 RITC 的权重是
> **0.5**、gross 上限 **300,000** —— 都和案例 PDF 写的(×2)不一致。
> 照 PDF 算套利容量会是 75,000 股,真实是 120,000 股。

确认无误后把 `DRY_RUN` 改成 `False`。

## 运行中:需要人工配合

大额 tender 无法只靠盘口平掉。实测一笔 83,000 股的 tender:

```
RITC 顶档买价 24.22 只有 12,200 股
RITC 盘口总深度 64,110 股 < 83,000
BULL 盘口总深度 41,969 股 —— 连一半都不到
```

**算法不能点换算器,只能你点。** 脚本接受 tender 时会打印:
...
看到就去 **Assets – Converters** 标签页按提示点击。每次换算:

| | 投入 | 产出 | 耗时 |
| --- | --- | --- | --- |
| ETF-Redemption | 10,000 RITC + 1,500 USD | 10,000 BULL + 10,000 BEAR | 2 tick |
| ETF-Creation | 10,000 BULL + 10,000 BEAR + 1,500 USD | 10,000 RITC | 2 tick |

成本 **0.151 CAD/股**,比三条腿的 0.06 手续费贵。**它不是省钱的路,是没有路时的路** ——
价值在于把出货压力分散到三个盘口而不是挤在一个。

> **不点会怎样:** 仓位平不掉,一直扛到收盘。脚本已经把换算计入了成交均价,
> 所以不点的话实际结果会比它算的差。

## 应急

和波动率案例一样:`python flatten.py --live` 不适用于 ETF 案例(它读的是波动率的配置)。
ETF 案例中途停下来,直接在 RIT Client 里平仓,或重启脚本。
